...
try {
  ...
  for (let run = 0; run < 2; run++) {
    ...
    assert(pnpm, "Run this check using pnpm test:bootstrap");
    const jsLauncher = /\.[cm]?js$/.test(pnpm);
    const result = spawnSync(
      jsLauncher ? process.execPath : pnpm,
      jsLauncher ? [pnpm, "db:migrate"] : ["db:migrate"],
